...
i= 1
df_list=vector("list", length = length(region.index))
while(i < length(region.index)+1){
  df_list[[i]]   = as.data.frame(df.region(df, region.index[i]))
  print(i)
  i = i+1
}

for (i in 1:length(df_list)){
names(df_list)[i] <- region.index[i]
}
```
...
grad_hess <- function(dat, beta) {
  ...
  c = beta[3]
  n = length(x)
  target = sum(y^2)+n*a^2+c^2*sum(exp(-2*b*x))-2*a*sum(y)-2*c*sum(y*exp(-b*x))+2*a*c*sum(exp(-b*x))
  grad <- c(2*n*a-2*sum(y)+2*c*sum(exp(-b*x)), c^2*sum(exp(-2*b*x)*(-2*x))-2*c*sum(y*exp(-b*x)*(-x))+2*a*c*sum(exp(-b*x)*(-x)), 2*c*sum(exp(-2*b*x))-2*sum(y*exp(-b*x))+2*a*sum(-b*x))
# gradient at betavec
  Hess <- matrix(c(2*n,
                    2*c*sum(exp(-b*x)*(-x)),
                    2*sum(exp(-b*x)),        
...
modified_NewtonRaphson <- function(dat,start, tol=1e-10,
  ...
  while(i < maxiter && abs(stuff$target - prevtarget) > tol)
 {
   ...
   stuff = grad_hess(dat,cur)
# step-halfing
   while(stuff$target <= prevtarget && l < 10){
     cur <- prev - (1/(2^l))*solve(stuff$Hess) %*% stuff$grad
     stuff = grad_hess(dat,cur)
     l= l+1
   }
...
beta = beta_trans(c(1000,0.1,40))
dat = dat_trans(df_list$Afghanistan)
