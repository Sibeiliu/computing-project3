...
beta = beta_trans(c(80000,0.3,50))
dat = dat_trans(dat_ori)
ans_mod = modified_NewtonRaphson(dat,beta)
...
ggplot(aes(x=time,y=cases),data=dat_ori)+
  geom_point()
beta = beta_trans(c(1000,0.08,30))
dat = dat_trans(dat_ori)
ans_mod = modified_NewtonRaphson(dat,beta)
...
em_dat = res %>% dplyr::select(-country_region) %>% 
  mutate(
    a_value = round(as.numeric(as.character(a_value)),0),
    b_value = round(as.numeric(as.character(b_value)),3),
    c_value = round(as.numeric(as.character(c_value)),0)
  ) 
